...
class SQLselect
{
    private const __count = 'count';
    private const __fetch = 'fetch';
    ...
    private static function SplitValues($return){
        try {
            self::$stmt = Connection::connect()->prepare(self::$sql);
            self::$stmt->execute();
            switch($return):
                case self::__count: return self::$stmt->rowCount(); break;
                case self::__fetch:
    ...
    public static function SELECT_ALL_ORDER_DESC_WHERE(string $table, $ref, $ref1, string $row, string $return){
