...
class SGBD extends Connection
{
     ...
     * @copyright tricndrd - Patrício Andrade All Rights Reserved
     * @license  MIT
     * @since 2020
     * @class SGBD START
     */
    ...
    private static function set_x_check(string $table){
        self::$x_value = Inspection::sql_count_all_in_table("$table");
    }
    #set y Verification Value
    private static function set_y_check(string $table){
        self::$y_value = Inspection::sql_count_all_in_table("$table");
    }
    ...
    #count_method
    private static function __RowCount(){
    ...
    public static function select_one_row(string $row, string $table){
        if(Inspection::sql_count_all_in_table("$table")):
            self::get_bool_state(true);
            return SQLselect::SELECT_1_ROW("$row", "$table",self::FetchData);
        else:
            self::get_bool_state(false);
        endif;
    }
    ...
    public static function select_all(string $table){
        if(Inspection::sql_count_all_in_table("$table")):
            self::get_bool_state(true);
            return SQLselect::SELECT_ALL("$table", self::FetchData);
        else:
            self::get_bool_state(false);
        endif;
    }
    /*all by one Reference*/
    public static function select_all_by_row_where_value(string $table, string $row, string $data){
        if(Inspection::sql_count_by_1_row("$table", "$row", "$data")):
            self::get_bool_state(true);
            return SQLselect::SELECT_ALL_WHERE("$table", "$row", "$data", self::FetchData);
        else:
            self::get_bool_state(false);
        endif;
    }
    ...
    public static function select_all_asc(string $table, string $row){
        if(Inspection::sql_count_all_in_table("$table")):
            self::setQuery(SQLselect::SELECT_ALL_ORDER_ASC("$table","$row",  self::FetchData));
            if (self::getQuery()):
                self::get_bool_state(true);
    ...
    public static function select_all_desc_limit(string $table, string $row, string $limit){
        if(Inspection::sql_count_all_in_table("$table")):
            self::setQuery(SQLselect::SELECT_ALL_ORDER_DESC_LIMIT("$table","$row",  "$limit",self::FetchData));
            if (self::getQuery()):
                self::get_bool_state(true);
                return self::getQuery();
            else:
    ...
    public static function select_all_asc_where(string $table, string $row, string $equal, string $order_by){
        if(Inspection::sql_count_all_in_table("$table")):
            self::setQuery(SQLselect::SELECT_ALL_ORDER_ASC_WHERE("$table","$row",  "$equal", "$order_by", self::FetchData));
            if (self::getQuery()):
                self::get_bool_state(true);
                return self::getQuery();
            else:
    ...
    public static function select_all_where_like(string $table, string $row, string $like){
        if(Inspection::sql_count_all_in_table("$table")):
            self::setQuery(SQLselect::SELECT_ALL_WHERE_LIKE("$table","$row",  "$like", self::FetchData));
            if (self::getQuery()):
                self::get_bool_state(true);
                return self::getQuery();
            else:
    ...
    public static function select_all_where_like_and(string $table, string $row, string $like){
        if(Inspection::sql_count_all_in_table("$table")):
            self::setQuery(SQLselect::SELECT_ALL_WHERE_AND_LIKE("$table","$row",  "$like", self::FetchData));
            if (self::getQuery()):
                self::get_bool_state(true);
                return self::getQuery();
            else:
    ...
    public static function select_one_where_limit(string $row, string $table, string $where, string $equal, string $limit){
        if(Inspection::sql_count_all_in_table("$table")):
            self::setQuery(SQLselect::SELECT_WHERE_LIMIT("$row", "$table", "$where", "$equal", "$limit", self::FetchData));
            if (self::getQuery()):
                self::get_bool_state(true);
                return self::getQuery();
            else:
    ...
    public static function select_all_desc_where_limit(string $table, string $row,string $equal, string $order_by, string $limit){
        if(Inspection::sql_count_all_in_table("$table")):
            self::setQuery(SQLselect::SELECT_ALL_ORDER_DESC_WHERE_LIMIT("$table","$row", "$equal", "$order_by", $limit, self::FetchData));
            if (self::getQuery()):
                self::get_bool_state(true);
                return self::getQuery();
            else:
    ...
    /*one*/
    public static function select_one(string $row, string $table, string $where, string $data){
        if(Inspection::sql_count_by_1_row("$table", "$where", "$data")):
            self::setQuery(SQLselect::SELECT("$row", "$table","$where", "$data", self::FetchData)[0]);
            if (self::getQuery()):
                self::get_bool_state(true);
                return self::getQuery();
            else:
    ...
    public static function select_one_where_2(string $row, string $table, string $where_row1, string $equal1, string $where_row2, string $equal2, string $data){
        if(Inspection::sql_count_by_1_row("$table", "$where_row1", "$equal1")):
            self::setQuery(SQLselect::SELECT_1_WHERE_2("$row", "$table","$where_row1","$equal1", "$where_row2", "$equal2", self::FetchData)[0]);
            if (self::getQuery()):
                self::get_bool_state(true);
                return self::getQuery();
            else:
    ...
    public static function select_all_where_2( string $table, string $where_row1, string $equal1, string $where_row2, string $equal2){
        if(Inspection::sql_count_by_1_row("$table", "$where_row1", "$equal1")):
            self::setQuery(SQLselect::SELECT_ALL_WHERE_2("$table", "$where_row1","$equal1", "$where_row2", "$equal2", self::FetchData));
            if (self::getQuery()):
                self::get_bool_state(true);
                return self::getQuery();
            else:
    ...
    public static function select_all_where_limit(string $table, string $row, string $equal_value, string $limit){
        if(Inspection::sql_count_by_1_row("$table", "$row", "$equal_value")):
            self::get_bool_state(true);
            self::setQuery(SQLselect::SELECT_ALL_WHERE_LIMIT("$table", "$row", "$equal_value", "$limit", self::FetchData));
            if (self::getQuery()):
                self::get_bool_state(true);
                return self::getQuery();
            else:
    ...
    /*twenty-four*/
    public static function insert_24(string $table, string $row1, string $row2,
                                     string $row3,  string $row4,
                                     string $row5, string $row6,
                                     string  $row7, string  $row8,
                                     string  $row9, string  $row10,
                                     string  $row11, string  $row12,
                                     string  $row13, string  $row14,
                                     string $row15, string $row16,
                                     string  $row17, string  $row18,
                                     string  $row19, string  $row20,
                                     string  $row21, string  $row22,
                                     string  $row23, string  $row24,
                                     string $value1,
                                     ...
                                     string  $value13,
                                     string $value14, string $value15, string $value16,
                                     string $value17, string $value18, string $value19,
                                     string $value20, string $value21, string $value22,
                                     string  $value23, string $value24
    ){
        if (Inspection::is_not_empty(array($table, $row1, $value1, $row2, $value2, $row3, $value3, $row4, $value4, $row5,$row6, $row7, $row8, $row9, $row10, $row11, $row12, $value5, $value6, $value7, $value8, $value9, $value10, $value11, $value12, $value13, $row13))):
            self::set_x_check($table);
                SQLinsert::INSERT_IN_24($table, $row1, $row2, $row3, $row4, $row5, $row6, $row7,
